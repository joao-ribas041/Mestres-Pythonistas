...
def login():
    ...
    if not auth or not auth.username or not auth.password:
        return make_response('Login inválido 1',401,{'WWW-AUTHENTICATE':'Basic realm="Login obrigatório"'})
    usuario = Autor.query.filter_by(nome=auth.username).first()
    if not usuario:
        return make_response('Login inválido 2 ',401,{'WWW-AUTHENTICATE':'Basic realm="Login obrigatório"'})
    if auth.password == usuario.senha:
        token = jwt.encode({'id_autor':usuario.id_autor,'exp':datetime.utcnow() + timedelta(minutes=30)},app.config['SECRET_KEY'])
        return jsonify({'token':token})
    return make_response('Login inválido 3',401,{'WWW-AUTHENTICATE':'Basic realm="Login obrigatório"'})
